...
res_mle <- optim(theta0, L_wrap2, , control = list(fnscale=-1), x=X, y1=y1, y2=y2, phi=5, delta1=delta1, delta2=delta2, n=n, hessian = TRUE)
...
#   prop.para <- current.para  
  ...
  if(current.xi1 < 0)
  {
    lb <- (min(current.mu1) - log(max(y1)))*current.xi1
    prop.sigma1 <- rtnorm(1,current.sigma1 ,propsd.sigma1,lower = lb, upper = Inf) 
  }
  ...
  if(is.infinite(loga) | is.nan(loga)) {
      print("Went wrong at sigma1")
      print(tt)
      print(loga)
      print(c(prop.mu1,prop.sigma1,prop.xi1,prop.mu2,prop.sigma2,prop.xi2))
  }
 ...
 if(u3<log_acc_prob) { 
  ...
  if(current.xi2 < 0)
  {
    lb <- (min(current.mu2) - log(max(y2)))*current.xi2
    prop.sigma2 <- rtnorm(1,current.sigma2 ,propsd.sigma2,lower = lb, upper = Inf) 
  }
  #random walk metropolis
  #fn...-log
...
file <- sprintf("~/Dooti/phi_8.RData")
save(PARA,file=file)
###########################################
