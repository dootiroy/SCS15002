...
#########Generating the data from simulation#############
n<-1000 #sample size
...
#plot the survival curve based on the GEV model with parameter xi.
fn0<-function(para){
 t<-y
 beta<-para[1:2]
 xi<-para[3]
...
pdf("C:/Users/dooti/Desktop/SurvivalCurveByMean2_Gev.pdf")
...
###### get MLE for data using GEV distribution#############
...
prop.s<-c(1,0.2,0.003) # sd of proposal normal for beta0, beta1, xi.
m<-10000
para0<-c(1.7,0.4, 0.3) #initial value for beta0, beta1, xi.
#para:beta0,beta1,xi
...
for (t in 1:m){  
    ...
    prop.para[3]<- rtnorm(1,current.para[3],prop.s[3],-Inf, ub)
  
    #random walk metropolis
    loga <- fn(prop.para)-fn(current.para)+ dtnorm(current.para[3], prop.para[3], prop.s[3],-Inf, ub,log=TRUE) - 
				  dtnorm(prop.para[3], current.para[3], prop.s[3],-Inf, ub,log=TRUE)
...
para.mle<-laplace(fn0,c(2,0.6,0.4))$mode
sd.mle<-sqrt(diag(laplace(fn0,c(2,0.6,0.001))$var))
#####para#################################################
