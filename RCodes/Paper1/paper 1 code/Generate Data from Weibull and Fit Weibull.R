...
library(LearnBayes)  
BETA<-laplace(fn0,c(2,0.6,1))$mode[1:2]
alpha<-laplace(fn0,c(2,0.6,1))$mode[3]
THETA<-exp(X%*%BETA)
...
sy<-apply(YYY,2,mean)
pdf("C:/Users/dooti/Desktop/SurvivalCurveByMean1_Weibull.pdf")
...
m<-10000
para0<-c(1.5,0.5,1) #initial value for beta0, beta1, alpha.
...
para.mle<-laplace(fn0,c(2,0.6,1))$mode
sd.mle<-sqrt(diag(laplace(fn0,c(2,0.6,1))$var))
#################################################
